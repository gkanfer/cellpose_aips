...
def display_image_label(ch,mask,lable_draw, font_select,font_size,windows=False):
    '''
...
def binary_output(mask,value,centroid_weighted=None,erosion_matrix=None):
    '''
    ...
    :return: binary mask
    '''
    mask_binary = np.zeros(np.shape(mask))
    if centroid_weighted is not None:
        if len(centroid_weighted) !=2:
            return 'centroid length is not of x and y'
        else:
            for z in range(len(centroid_weighted['x'])):
                mask_binary[int(centroid_weighted['x'][z]),int(centroid_weighted['y'][z])] = 1
                return mask_binary
    else:
        for i in range(len(value)):
            mask_binary[mask==value[i]]=1
            if erosion_matrix is not None:
                mask_binary = binary_erosion(mask_binary, structure=np.ones((erosion_matrix, erosion_matrix))).astype(np.float64)
    return mask_binary
