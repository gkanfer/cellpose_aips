...
class AIPS_cellpose:
    ...
    def call_bin(self,table_sel_cor, threshold ,img_blank):
        ...
        for z in range(len(table_na_rmv_trgt)):
            image_blank_temp = np.zeros_like(img_blank)
            x, y = table_na_rmv_trgt.loc[table_na_rmv_trgt.index[z],["centroid-0", "centroid-1"]]
            row, col = disk((int(y), int(x)), 10,shape=np.shape(img_blank))
            image_blank_temp[col, row] = 1
            img_blank = img_blank + image_blank_temp
        # seg_mask_eros_9 = binary_erosion(img_blank, structure=np.ones((9, 9))).astype(np.float64)
        # seg_mask_eros_3 = binary_erosion(img_blank, structure=np.ones((3, 3))).astype(np.float64)
