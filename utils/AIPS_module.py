...
class AIPS:
    ...
    def load_image(self):
        ...
        if np.shape(pixels_float)[2]==3:
            c1 = pixels_float[:, :, 0]
            c1 = c1.reshape(1, np.shape(c1)[0], np.shape(c1)[1])
            c2 = pixels_float[:, :, 1]
            c2 = c2.reshape(1, np.shape(c2)[0], np.shape(c2)[1])
            c3 = pixels_float[:, :, 2]
            c3 = c3.reshape(1, np.shape(c3)[0], np.shape(c3)[1])
            pixels_float = np.concatenate((c1, c2, c3), 0)
